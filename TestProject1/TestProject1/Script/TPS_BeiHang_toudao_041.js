...
function changeUserPasswordThenCancel(loginUserName, loginPassword, newUserName, newUserPassword, editUserPassword) {    
    ...
    if (!strictEqual(ret, -1)) {
         ...
         logout.logout();
         //check new user can login with original password
         login.login(newUserName, newUserPassword);
         logout.logout();
         //login for deleting dirty data
         login.login(loginUserName, loginPassword);
         indel.PatientManagementWidget.groupBox.frame.pushButton_UserManage.ClickButton();
     } else {
